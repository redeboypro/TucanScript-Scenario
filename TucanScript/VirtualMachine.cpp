...
VM::Val* TucanScript::VM::VirtualMachine::GetMemoryAtAddress (const Val& src, Val* defaultValue) const {
	if (src.m_Type == RADDRESS_T) {
		return GetMemoryAtRAddress (src.m_Data.m_I32);
	}
	else if (src.m_Type == LRADDRESS_T) {
		return GetMemoryAtLRAddress (src.m_Data.m_I32);
	}
	return defaultValue;
}

Undef TucanScript::VM::VirtualMachine::StrOp (const Val& a, const Val& b, Sym* (*op)(Sym*, const Sym*)) {
	auto* aStr = (Sym*) (a.m_Data.m_NativePtr);
	auto* bStr = (Sym*) (b.m_Data.m_NativePtr);
	m_Stack.Push <Undef*, NATIVEPTR_T> (op (aStr, bStr), &Word::m_NativePtr);
}
...
Undef TucanScript::VM::VirtualMachine::AllocStr (Sym* buffer, Size size) {
...
Undef TucanScript::VM::VirtualMachine::Run (SInt32 entryPoint) {
	for (Size iInst = entryPoint; iInst < m_Asm.m_Size; ++iInst) {
		auto& instruction = m_Asm.m_Memory[iInst];
		switch (instruction.m_Op) {
			...
			case PUSH: {
				m_Stack.Push (instruction.m_Val);
				break;
			}
			case POP: {
				m_Stack.Pop ();
			...
			case JMPR: {
				GetLastCall ().m_Address = NextWord (iInst);
				m_JmpMemory.m_Depth++;
				SInt32 callMemorySize = PopUnpack ().m_Data.m_I32;
				const SInt32 numArgs = PopUnpack ().m_Data.m_I32;
				auto& callMemory = GetLastCall ().m_Memory;
				{
			...
			case RETURN: {
				...
				m_JmpMemory.m_Depth--;
				iInst = PrevWord (GetLastCall ().m_Address);
				break;
			}
			case MEMSIZE: {
				auto src = PopUnpack ();
			...
			case CMEMALLOC: {
				m_Stack.Push (Val {
					...
					.m_Data =
						Word {
							.m_NativePtr = std::malloc (PopUnpack ().m_Data.m_U64)
						}
				});
				break;
			}
			case MEMALLOC: {
				m_Stack.Push (Val {
					...
					.m_Data =
						Word {
							.m_ManagedPtr = m_Allocator.Alloc (PopUnpack ().m_Data.m_U64)
						}
				});
				break;
			}
			case MEMAPPEND: {
				auto newElement = PopUnpack ();
				auto memoryHolder = PopUnpack ();
			...
			case MEMDEALLOC: {
				...
				if (poppedValue.m_Type Is RADDRESS_T) {
					...
					FreeManagedMemory (GetMemoryAtLRAddress (poppedValue.m_Data.m_I32));
					break;
				}
			...
			case WRAP: {
				auto* memory = GetMemoryAtAddress (m_Stack.Pop (), nullptr);
				if (!memory) {
					LogInstErr (nameof (WRAP), "Invalid destination address!");
					Free ();
					return;
				}
				m_Stack.Push <Undef*, NATIVEPTR_T> (&memory->m_Data, &Word::m_NativePtr);
				break;
			}
			case CONCAT: {
				auto b = PopUnpack ();
				...
				m_Stack.Push <Managed*, MANAGED_T> (managedMemory, &Word::m_ManagedPtr);
				break;
			}
			case STRCAT: {
				auto b = PopUnpack ();
				auto a = PopUnpack ();
				StrOp (a, b, &std::strcat);
				break;
			}
			case STRCPY: {
				auto b = PopUnpack ();
